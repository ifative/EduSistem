...
export default function SubjectsCreate({ groups }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('subjects.create')} />
            <div className="flex flex-col gap-4 p-4">
                <h1 className="text-2xl font-bold">{t('subjects.create')}</h1>
                <Card>
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex gap-4 pt-4"><Button type="submit" disabled={processing}>{processing ? t('subjects.creating') : t('subjects.create')}</Button><Link href="/admin/master/subjects"><Button type="button" variant="outline">{t('common:actions.cancel')}</Button></Link></div>
                        </form>
                    </CardContent>
                </Card>
            </div>
