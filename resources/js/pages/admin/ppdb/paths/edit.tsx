...
import { index, update } from '@/routes/admin/ppdb/paths';
...
export default function PathsEdit({ path, periods }: Props) {
    const { t } = useTranslation(['admin', 'common']);
    const { data, setData, put, processing, errors } = useForm({
        admission_period_id: String(path.admission_period_id),
        name: path.name,
        code: path.code,
        description: path.description || '',
        type: path.type,
        quota: String(path.quota),
        min_score: path.min_score !== null ? String(path.min_score) : '',
        max_distance: path.max_distance !== null ? String(path.max_distance) : '',
        requires_test: path.requires_test,
        requires_documents: path.requires_documents,
        selection_criteria: path.selection_criteria ? JSON.stringify(path.selection_criteria) : '{}',
        sort_order: String(path.sort_order),
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(update.url(path.id));
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('admin:ppdb.paths.edit')} />
            <div className="flex flex-col gap-4 p-4">
                <h1 className="text-2xl font-bold">{t('admin:ppdb.paths.edit')}</h1>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-4">
                        <Button type="submit" disabled={processing}>
                            {processing ? t('admin:ppdb.paths.updating') : t('common:actions.save')}
                        </Button>
                        <Link href={index.url()}>
