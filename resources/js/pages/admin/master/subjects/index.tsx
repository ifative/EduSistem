...
export default function SubjectsIndex({ subjects, filters, groups }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('subjects.title')} />
            <div className="flex flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">{t('subjects.title')}</h1>
                    <Link href="/admin/master/subjects/create"><Button><PlusIcon className="mr-2 h-4 w-4" />{t('subjects.add')}</Button></Link>
                </div>
                <Card>
                    <CardHeader>
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <CardTitle>{t('subjects.all_subjects')}</CardTitle>
                            <div className="flex flex-wrap gap-2">
                                <Input placeholder={t('subjects.search_placeholder')} value={search} onChange={(e) => setSearch(e.target.value)} className="w-48" />
                                <Select value={group} onValueChange={setGroup}><SelectTrigger className="w-32"><SelectValue placeholder={t('subjects.group')} /></SelectTrigger><SelectContent><SelectItem value="__all__">{t('subjects.all_groups')}</SelectItem>{groups.map((g) => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent></Select>
                    ...
                    <CardContent>
                        <Table>
                            <TableHeader><TableRow><TableHead>{t('subjects.code')}</TableHead><TableHead>{t('subjects.name')}</TableHead><TableHead>{t('subjects.group')}</TableHead><TableHead>{t('subjects.credits')}</TableHead><TableHead className="text-right">{t('common:table.actions')}</TableHead></TableRow></TableHeader>
                            <TableBody>
                                {subjects.data.map((s) => (
                                    <TableRow key={s.id}>
                                ...
                                {subjects.data.length === 0 && <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">{t('subjects.empty_title')}</TableCell></TableRow>}
                            </TableBody>
                        </Table>
                    </CardContent>
                </Card>
            </div>
            <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t('subjects.delete_title')}</AlertDialogTitle><AlertDialogDescription>{t('subjects.delete_description')}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t('common:actions.cancel')}</AlertDialogCancel><AlertDialogAction onClick={handleDelete}>{t('common:dialog.delete')}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
        </AppLayout>
    );
}
